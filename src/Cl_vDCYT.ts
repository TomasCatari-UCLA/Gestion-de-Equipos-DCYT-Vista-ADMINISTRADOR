...
export default class Cl_vDCYT extends Cl_vGeneral {
  ...
  private lblOperativos: HTMLElement;
  private lblReparacion: HTMLElement;
  private lblDañado: HTMLElement;

  private modalEliminar: HTMLElement;
  private btConfirmarSi: HTMLButtonElement;
  private btConfirmarNo: HTMLButtonElement;
  private serialPendiente: string | null = null;

  // --- ELEMENTOS DE BÚSQUEDA ---
  private modalBuscar: HTMLElement;
  private inBusSerial: HTMLInputElement;

  // CAMBIO: Ya no son HTMLSelectElement, ahora son HTMLDivElement (Contenedores)
  ...
  constructor() {
    ...
    this.lblOperativos = document.getElementById(
      "lblOperativos"
    ) as HTMLElement;
    this.lblReparacion = document.getElementById(
      "lblReparacion"
    ) as HTMLElement;
    this.lblDañado = document.getElementById("lblDañado") as HTMLElement;

    this.modalEliminar = document.getElementById(
      "modalEliminar"
    ) as HTMLElement;
    this.btConfirmarSi = document.getElementById(
      "btConfirmarSi"
    ) as HTMLButtonElement;
    this.btConfirmarSi.onclick = () => this.ejecutarBorrado();
    this.btConfirmarNo = document.getElementById(
      "btConfirmarNo"
    ) as HTMLButtonElement;
    ...
    this.inBusSerial = document.getElementById(
      "bus_inSerial"
    ) as HTMLInputElement;
    ...
    this.inBusPuesto = document.getElementById(
      "bus_inPuesto"
    ) as HTMLInputElement;
    ...
    this.divBusEstado = document.getElementById(
      "bus_divEstado"
    ) as HTMLDivElement;
    ...
    this.btBuscarCancelar = document.getElementById(
      "btBuscarCancelar"
    ) as HTMLButtonElement;
    this.btBuscarCancelar.onclick = () => this.ocultarBusqueda();
    this.btBuscarAceptar = document.getElementById(
      "btBuscarAceptar"
    ) as HTMLButtonElement;
  ...
  mostrarEquipos(listaFiltrada?: iEquipo[]) {
    ...
    equipos.forEach((equipo: iEquipo, index: number) => {
      ...
      html += `<div class="card">
                <div class="card-content">
                    <div class="card-title">${equipo.lab} Fila:${
        equipo.fila
      } Puesto:${equipo.puesto}</div>
                    <div class="card-detail"> <b>Serial:</b> ${
                      equipo.serial || "N/A"
                    } | <b> Equipo:</b> ${equipo.cpu} - <b> RAM:</b> ${
        equipo.ram
      }GB</div>
                    <div class="card-status ${claseColor}"><span class="status-dot">●</span> ${
        equipo.estado
      }</div>
                </div>
    ...
    equipos.forEach((equipo: iEquipo, index: number) => {
      ...
      if (btnE) btnE.onclick = () => this.editarEquipo(equipo.serial);
      let btnD = document.getElementById(`dcyt_btEliminar_${index}`);
      if (btnD) btnD.onclick = () => this.pedirConfirmacion(equipo.serial);
    });
  }
  ...
  pedirConfirmacion(serial: string) {
